你现在只做 README_V1.5.md 的创建与只读核验，不要 git commit，不要 git push，不要修改其他文件。

执行要求：
1. 先确认当前仓库根目录、当前分支、git status
2. 在仓库根目录创建 README_V1.5.md
3. 文件内容必须严格按我提供的 Markdown 写入
4. 写入完成后，只返回：
   - REPO_ROOT=
   - BRANCH=
   - FILE_EXISTS=YES/NO
